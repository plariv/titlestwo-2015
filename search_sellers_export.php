...
$layout->blocks["top"][] = "export";$page_layouts["search_sellers_export"] = $layout;
...
$xt->display("search_sellers_export.htm");

function ExportToExcel_old()
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=search_sellers.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=search_sellers.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=search_sellers.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		...
		$values = array();
			...
			$values["seller_name"] = GetData($row,"seller_name","");
			$values["ttl_actions"] = GetData($row,"ttl_actions","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=search_sellers.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$format = "";
			$values["seller_name"] = GetData($row,"seller_name",$format);
			$format = "";
			$values["ttl_actions"] = GetData($row,"ttl_actions",$format);
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["seller_name"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["ttl_actions"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Seller Name").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Ttl Actions").'</td>';	
	}
	else
	{
		echo "<td>"."Id"."</td>";
		echo "<td>"."Seller Name"."</td>";
		echo "<td>"."Ttl Actions"."</td>";
	}
	echo "</tr>";
			$totals = array();
		$totals["id"] = array("value" => 0, "numRows" => 0);
		$totalsFields[] = array('fName'=>"id", 'totalsType'=>'', 'viewFormat'=>"");
			$totals["seller_name"] = array("value" => 0, "numRows" => 0);
		$totalsFields[] = array('fName'=>"seller_name", 'totalsType'=>'', 'viewFormat'=>"");
			$totals["ttl_actions"] = array("value" => 0, "numRows" => 0);
		$totalsFields[] = array('fName'=>"ttl_actions", 'totalsType'=>'', 'viewFormat'=>"");
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["seller_name"]);
					else
						echo htmlspecialchars($values["seller_name"]);
			echo '</td>';
							echo '<td>';
			
			
									$format="";
									echo htmlspecialchars($values["ttl_actions"]);
			echo '</td>';
			echo "</tr>";
		}
