...
$layout->blocks["top"][] = "details";$page_layouts["search_sellers_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "search_sellers_inline_edit.htm" : "search_sellers_edit.htm";
...
//	processing seller_name - begin
	$condition = 1;

	if($condition)
	{
		$value = postvalue("value_seller_name_".$id);
		$type = postvalue("type_seller_name_".$id);
		if(FieldSubmitted("seller_name_".$id))
		{
				$value = prepare_for_db("seller_name",$value,$type);
		}
		else
			$value = false;
		...
		{	
			...
			if(0 && "seller_name"=="password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["seller_name"] = $value;
		
			}
	
		}
//	processing seller_name - end
//	processing ttl_actions - begin
	$condition = 1;

	if($condition)
	{
		$value = postvalue("value_ttl_actions_".$id);
		$type = postvalue("type_ttl_actions_".$id);
		if(FieldSubmitted("ttl_actions_".$id))
		{
				$value = prepare_for_db("ttl_actions",$value,$type);
		}
		else
			$value = false;
		...
		{	
			...
			if(0 && "ttl_actions"=="password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["ttl_actions"] = $value;
		
			}
	
		}
//	processing ttl_actions - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: search_sellers_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_search_sellers->Copy();
...
if(!$data)
{
	...
	{
		header("Location: search_sellers_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["seller_name"] = $evalues["seller_name"];
	$data["ttl_actions"] = $evalues["ttl_actions"];
}
...
//	seller_name - 
		...
		$value = "";
			$value = ProcessLargeText(GetData($data,"seller_name", ""),"field=seller%5Fname".$keylink,"",MODE_LIST);
	$showValues["seller_name"] = $value;
	$showFields[] = "seller_name";
		$showRawValues["seller_name"] = substr($data["seller_name"],0,100);

//	ttl_actions - 

		$value = "";
			$value = ProcessLargeText(GetData($data,"ttl_actions", ""),"field=ttl%5Factions".$keylink,"",MODE_LIST);
	$showValues["ttl_actions"] = $value;
	$showFields[] = "ttl_actions";
		$showRawValues["ttl_actions"] = substr($data["ttl_actions"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "search_sellers";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='search_sellers_view.php?".$viewlink."'\"");
