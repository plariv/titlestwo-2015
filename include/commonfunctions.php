...
function checkTableName($shortTName, $type=false)
{
	...
	if ("agents" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("ttl_actions" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("brokers" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("buyers" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("parties" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("properties" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("sellers" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("ttl_recordings" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("ttlaction_notes" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("lastfilenumbers" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("search5" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("ttwousers" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("titlestwo1_uggroups" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("titlestwo1_ugmembers" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("titlestwo1_ugrights" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	...
	if ("sellersactions" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("buyersactions" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("search_sellers" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	return false;
...
function GetLWWhere($field,$table="")
{
	...
	if(!$table) 
		$table = $strTableName;
	if($table=="ttl_actions" && $field=="propaddr1")
		return "propaddr1 <> ''";
	return "";
}
...
{
	$arr=array();
	$strPerm = GetUserPermissions("agents");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="agents";
	}
	$strPerm = GetUserPermissions("ttl_actions");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="ttl_actions";
	}
	$strPerm = GetUserPermissions("brokers");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="brokers";
	}
	$strPerm = GetUserPermissions("buyers");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="buyers";
	}
	$strPerm = GetUserPermissions("parties");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="parties";
	}
	$strPerm = GetUserPermissions("properties");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="properties";
	}
	$strPerm = GetUserPermissions("sellers");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="sellers";
	}
	$strPerm = GetUserPermissions("ttl_recordings");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="ttl_recordings";
	}
	$strPerm = GetUserPermissions("ttlaction_notes");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="ttlaction_notes";
	}
	$strPerm = GetUserPermissions("lastfilenumbers");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="lastfilenumbers";
	}
	$strPerm = GetUserPermissions("search5");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="search5";
	}
	$strPerm = GetUserPermissions("ttwousers");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="ttwousers";
	}
	$strPerm = GetUserPermissions("titlestwo1_uggroups");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="titlestwo1_uggroups";
	}
	$strPerm = GetUserPermissions("titlestwo1_ugmembers");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="titlestwo1_ugmembers";
	}
	$strPerm = GetUserPermissions("titlestwo1_ugrights");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="titlestwo1_ugrights";
	}
	...
	$strPerm = GetUserPermissions("sellersactions");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="sellersactions";
	}
	$strPerm = GetUserPermissions("buyersactions");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="buyersactions";
	}
	$strPerm = GetUserPermissions("search_sellers");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="search_sellers";
	}
...
function GetTableURL($table="")
{
	...
	if("sellers"==$table) 
		return "sellers";
	if("ttl_recordings"==$table) 
		return "ttl_recordings";
	if("ttlaction_notes"==$table) 
		return "ttlaction_notes";
	if("lastfilenumbers"==$table) 
		return "lastfilenumbers";
	if("search5"==$table) 
		return "search5";
	if("ttwousers"==$table) 
		return "ttwousers";
	if("titlestwo1_uggroups"==$table) 
		return "titlestwo1_uggroups";
	if("titlestwo1_ugmembers"==$table) 
		return "titlestwo1_ugmembers";
	if("titlestwo1_ugrights"==$table) 
		return "titlestwo1_ugrights";
	...
	if("admin_users"==$table) 
		return "admin_users";
	if("sellersactions"==$table) 
		return "sellersactions";
	if("buyersactions"==$table) 
		return "buyersactions";
	if("search_sellers"==$table) 
...
{	
	if(!$shortTName)
		return false;
	if("agents"==$shortTName) 
		return "agents";
	if("ttl_actions"==$shortTName) 
		return "ttl_actions";
	if("brokers"==$shortTName) 
		return "brokers";
	if("buyers"==$shortTName) 
		return "buyers";
	if("parties"==$shortTName) 
		return "parties";
	if("properties"==$shortTName) 
		return "properties";
	if("sellers"==$shortTName) 
		return "sellers";
	if("ttl_recordings"==$shortTName) 
		return "ttl_recordings";
	if("ttlaction_notes"==$shortTName) 
		return "ttlaction_notes";
	if("lastfilenumbers"==$shortTName) 
		return "lastfilenumbers";
	if("search5"==$shortTName) 
		return "search5";
	if("ttwousers"==$shortTName) 
		return "ttwousers";
	if("titlestwo1_uggroups"==$shortTName) 
		return "titlestwo1_uggroups";
	if("titlestwo1_ugmembers"==$shortTName) 
		return "titlestwo1_ugmembers";
	if("titlestwo1_ugrights"==$shortTName) 
		return "titlestwo1_ugrights";
	...
	if("admin_users"==$shortTName) 
		return "admin_users";
	if("sellersactions"==$shortTName) 
		return "sellersactions";
	if("buyersactions"==$shortTName) 
		return "buyersactions";
	if("search_sellers"==$shortTName) 
		return "search_sellers";
}
...
function ReadUserPermissions($userID="")
{
	...
	if($userID!="Guest")
	{
		$sql = "select GroupID from `titlestwo1_ugmembers` where UserName=".db_prepare_string($userID);
		$rs = db_query($sql,$conn);
	...
	$rights=array();
	$sql = "select TableName,AccessMask from `titlestwo1_ugrights` where GroupID in (".$groupstr.")";
	$rs = db_query($sql,$conn);
	while($data=db_fetch_numarray($rs))
	{
...
{	
	...
	if(!count($_SESSION["UserRights"]["Guest"]))
		return false;
	if(array_key_exists("agents",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("ttl_actions",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("brokers",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("buyers",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("parties",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("properties",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("sellers",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("ttl_recordings",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("ttlaction_notes",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("lastfilenumbers",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("search5",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("ttwousers",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("titlestwo1_uggroups",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("titlestwo1_ugmembers",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("titlestwo1_ugrights",$_SESSION["UserRights"]["Guest"]))
		return true;
	...
	if(array_key_exists("admin_users",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("sellersactions",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("buyersactions",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("search_sellers",$_SESSION["UserRights"]["Guest"]))
		return true;
	return false;
}
...
function BuildRadioControl($field, $value,$fieldNum=0,$id="", $mode)
{
	...
	$LookupSQL ="";
	$spacer = '<br/>';
	if($strTableName=="ttl_actions" && "staffatty"==$field)
	{
					$arr=array();
			$arr[]="RCM";
			$arr[]="MBG";
