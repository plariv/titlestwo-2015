...
$layout->blocks["top"][] = "details";$page_layouts["search_sellers_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "search_sellers_inline_add.htm";
else
	$templatefile = "search_sellers_add.htm";
...
//	processing seller_name - start
	...
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["seller_name"]=$value;
		}
		}
//	processibng seller_name - end
//	processing ttl_actions - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["ttl_actions"]=$value;
		}
		}
//	processibng ttl_actions - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'search_sellers_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'search_sellers_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: search_sellers_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["seller_name"]=@$avalues["seller_name"];
	$defvalues["ttl_actions"]=@$avalues["ttl_actions"];
}
